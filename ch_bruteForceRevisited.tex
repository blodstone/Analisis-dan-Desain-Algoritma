...
\begin{equation}
    ...
    \begin{split}
	f(n) & = f(n - 1) + f(n - 2) + f(n - 3) + \cdots + f(n - k)
    \end{split}
\end{equation}
...
\begin{equation}
    ...
    \begin{split}
	f(n) & = f(n - 1) + f(n - 2) + f(n - 3) + \cdots + f(n - k) \\
	     & = \big(f(n - 2) + f(n - 3) + \cdots + f(n - k)\big) + f(n - 2) + f(n - 3) + \cdots + f(n - k) \\
	     & = 2 \big(f(n - 2) + f(n - 3) + \cdots + f(n - k)\big) \\
	     & = 2 \big( \big( f(n - 3) + \cdots + f(n - k) \big) + f(n - 3) + \cdots + f(n - k)\big) \\
	     & = 2 \big( 2 \big( f(n - 3) + \cdots + f(n - k) \big)\big) \\
	     & = 2^k \big(f(n - k)\big)
    \end{split}
\end{equation}

Dengan menggunakan persamaan~\ref{eq:permutasi-proof-1} maka didapatkan:

\begin{equation}
    \label{eq:permutasi-proof-3}
    \begin{split}
	f(n) & = 2^{n - 1} \big(f(1)\big) \\
	     & = 2^{n - 1}
    \end{split}
\end{equation}

Dan karena notasi big O yang menyederhanakan, maka didapatkan kompleksitas algoritma~\ref{algo:permutasi} sebesar $\pmb{O(2^n)}$.
